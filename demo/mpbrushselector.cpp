...
MPBrushSelector::MPBrushSelector(const QString &brushLibPath, QWidget *p_parent): QTabWidget(p_parent), m_brushesPath(brushLibPath)
{
        ...
        while(!fileOrder.atEnd()) {
            ...
            if (line.startsWith("Group:")) {
                ...
                if (!currentGroup.isEmpty() && !brushesGroup.isEmpty())
                    m_brushLib.insert(currentGroup, brushesGroup);
...
void MPBrushSelector::selectBrush (QString brushName)
{
    ...
    if (!p_item && p_page && p_page->count())
        p_item = p_page->item(0);
